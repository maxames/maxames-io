...
image: ../assets/ashby-slack-feedback.svg
info:
  - text: GitHub
...
Most recruiting tools use Slack for notifications or simple approvals. This treats Slack as a full feedback surface with dynamic form generation.

**Built in ~3 days using FastAPI, AsyncPG, and the Slack SDK.** Pyright strict mode for type safety. Deployment guides included for Docker, Render, Railway, Fly.io, and manual installation.
...
When interview schedules update, the system deletes old records and inserts new ones rather than attempting to diff changes. This is simpler to implement and safer for webhook idempotency—duplicate webhooks result in the same final state regardless of order.

Trade-off: Slightly more database writes, but eliminates an entire class of sync bugs.
...
Single-instance only—horizontal scaling would require distributed lock (Redis) or external cron triggering admin endpoints. Unlikely to be necessary unless interview volume exceeds several hundred per hour.

### Idempotency

**Webhooks**: Full-replace strategy ensures identical final state regardless of duplicates.
...
**Type checking**: Pyright strict mode, zero type errors. All external API responses typed at boundaries using TypedDict. Services and API handlers work with typed structures throughout.
...
- **Manual** - systemd + nginx + Let's Encrypt
...
Open source. No production deployment yet. One component of broader recruiting automation work.

This addresses feedback collection timing and surface. Other friction points—reminder escalation, feedback quality signals, submission analytics—are separate problems that would require additional tooling.

## Limitations

**Single-instance scheduler**: APScheduler runs in-process. Horizontal scaling would require distributed lock (Redis/PostgreSQL advisory locks) or external job trigger (cron calling admin endpoints).

**No retry logic for failed Slack messages**: If Slack API call fails, reminder is marked as sent to prevent duplicate sends. Manual retry via admin endpoint required.

**Timezone handling**: All timestamps stored as UTC. Slack converts to user timezone automatically. Ashby API assumes UTC for ISO 8601 strings with 'Z' suffix.
